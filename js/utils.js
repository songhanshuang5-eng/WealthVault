...
// ── 数据结构 ──────────────────────────────────────────────
const EMPTY={accounts:[],investments:[],fxHistory:[],transfers:[],snapshots:[]};

// ── 工具函数 ──────────────────────────────────────────────
const genId=()=>Date.now().toString(36)+Math.random().toString(36).slice(2,6);
const fmtNum=(n,d=2)=>n==null||isNaN(n)?'—':Number(n).toLocaleString('zh-CN',{minimumFractionDigits:d,maximumFractionDigits:d});
...
// ── 数据持久化 ────────────────────────────────────────────
const STORE_KEY='pab_v1';
const loadData=()=>{try{const s=localStorage.getItem(STORE_KEY);const d=s?JSON.parse(s):EMPTY;return{...EMPTY,...d};}catch{return EMPTY;}};
...
const calcEffectivePosition=inv=>{
  ...
  let totalCost=avgCost*totalQty;
  let realizedPL=0;
  ...
  sorted.forEach(t=>{
    ...
    if(t.type==='buy'){
      totalCost+=qty*price;totalQty+=qty;
      avgCost=totalQty>0?totalCost/totalQty:avgCost;
    }else if(t.type==='sell'){
      realizedPL+=qty*(price-avgCost);
      totalQty=Math.max(0,totalQty-qty);
      totalCost=avgCost*totalQty;
    }
  });
  return{effectiveQty:totalQty,avgCost,realizedPL,totalCost};
};
